#
# This file by  katbailey.github.io
# Original: http://katbailey.github.io/post/matrix-factorization-with-tensorflow/
#
...
def mf(ratings_train, ratings_val, W, H, regularizer, mean_rating, max_iter, lr = 0.01, decay_lr = False, log_summaries = False):
    ...
    loga("num_ratings_tr: {0:d} user_indices_tr: {1} item_indices_tr: {2}".format(num_ratings_tr, user_indices_tr, item_indices_tr))
    ...
    if log_summaries:
        ...
        accuracy_val_summary = tf.summary.scalar("cost", cost)
        #accuracy_tr_summary = tf.summary.scalar("accuracy_tr", tr)
        summary_op = tf.summary.merge_all()
        writer = tf.summary.FileWriter("/tmp/recommender_logs", sess.graph)
    # Keep track of cost difference.
    last_cost = 0
    diff = 1
    # Run the graph and see how we're doing on every 100th iteration.
    for i in range(max_iter):
        if i > 0 and i % 30 == 0:
            if diff < 0.000001:
                loga("Converged at iteration %s" % (i))
                break;
            if log_summaries:
            ...
            acc_val = res[1]
            cost_ev = res[2]
            loga("Training RMSE at step %s: %s" % (i, acc_tr))
            loga("Validation RMSE at step %s: %s" % (i, acc_val))
...
def create_factors_with_biases(user_bias, item_bias, rank, lda):
    ...
    #W_plus_bias = tf.concat([W, tf.ones((num_users,1), dtype=float32, name="item_bias_ones")], 1)
...
def learn_bias_lda(ratings, num_folds, ldas, num_users, num_items, global_mean, max_iter, user_bias = True):
    ...
    for lda in ldas:
        ...
        for train, test in skf:
            tr, val, finalw, finalh = mf(ratings[train,:], ratings[test,:], W, H, reg, global_mean, max_iter, 0.8, log_summaries = True)
            sum_rmses += val
            loga("Training rmse: %s, val rmse: %s, lda: %s" % (tr, val, lda))
...
def get_user_bias(ratings_tr, ratings_val, lda, num_users, num_items, global_mean, max_iter):
    ...
    tr, val, finalw, finalh = mf(ratings_tr, ratings_val, W, H, reg, global_mean, max_iter, 0.8, log_summaries = True)
    return finalw
...
def main():
    ...
    loga("reading feather file from: %s" % (path))
    ratings_df = feather.read_dataframe(path)
    num_ratings = ratings_df.shape[0]
    loga("Dataframe read: %d entries" % (num_ratings))
    ...
    loga("ratings concatenated with all-one bias columns/rows")
    global_mean = mean(ratings[:,2])
    loga("Global_mean: %.2f" % (global_mean))
    np.random.seed(12)
    loga("Splitting into ratings and training sets")
    ratings_tr, ratings_val = train_test_split(ratings, train_size = .7)
    #loga("ratings: training: {0:s} validation: {1:s}".format(ratings_tr, ratings_val))
    ...
    if to_learn == "user_bias_lda":
        ...
        loga("Final training RMSE %s" % (tr))
        loga("Final validation RMSE %s" % (val))
        ...
        tr, val, finalw, finalh = mf(ratings_tr, ratings_val, W, H, reg, global_mean, max_iter, 1.0, True, log_summaries = True)
        loga("Final training RMSE %s" % (tr))
        loga("Final validation RMSE %s" % (val))
        ...
        tr, val, finalw, finalh = mf(ratings_tr, ratings_val, W, H, reg, global_mean, max_iter, 100.0, True, log_summaries = True)
        loga("Final training RMSE %s" % (tr))
        loga("Final validation RMSE %s" % (val))
        np.save("final_w", finalw)
        np.save("final_h", finalh)
        np.savetxt("final_w", finalw)
        np.savetxt("final_h", finalh)
