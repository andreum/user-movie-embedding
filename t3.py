...
def data_loading(fname):
   ...
   return key, value, userId, movieId, rating, timestamp
...
key, value, userId, movieId, rating, timestamp = data_loading("ratings-headerless.csv")
features = tf.stack([userId, movieId])

# see Using the Dataset API for TensorFlow Input Pipelines
def input_fn(dataset):
  userId = tf.feature_column.numeric_column("userId") # mb it shouldn't be numeric?
